...
namespace MessageBus.RabbitMq
{
    public class RabbitMqEventConsumer : IEventConsumer
    {
        ...
        private readonly ConcurrentDictionary<string, ISet<Type>> _availableHandlers = new ConcurrentDictionary<string, ISet<Type>>();
        private readonly ConcurrentDictionary<string, Type> _availableEventTypes = new ConcurrentDictionary<string, Type>();
        ...
        internal async Task OnReceived(object sender, BasicDeliverEventArgs @event)
        {
            ...
            if (@event.BasicProperties is null || string.IsNullOrWhiteSpace(@event.BasicProperties.Type))
            {
                _logger.LogError($"Message handling failed. Message did not contain a valid {nameof(IBasicProperties.Type)}");
                return;
            }
            ...
            where TEventHandler : IEventHandler<TEvent>
        {
            ...
            if (!handlers.Contains(typeof(TEventHandler)))
            {
